...
type CreateGitStackInput struct {
	Body struct {
		Name        string `json:"name" minLength:"1" maxLength:"128" pattern:"^[A-Za-z0-9_-]+$" doc:"Stack name"`
		...
		SSHKeyFile  string `json:"ssh_key_file,omitempty" maxLength:"512" doc:"Path to SSH key file on server (per-stack override)"`
		...
		AgeKey      string `json:"age_key,omitempty" maxLength:"4096" doc:"Per-stack age private key for SOPS decryption (overrides global)"`
	}
}
...
type UpdateStackInput struct {
	Name string `path:"name" maxLength:"128" doc:"Stack name"`
	Body struct {
		Compose string `json:"compose" minLength:"1" maxLength:"524288" doc:"Updated compose.yaml content (max 512 KB)"`
	}
}
...
type UpdateEnvInput struct {
	Name string `path:"name" maxLength:"128" doc:"Stack name"`
	Body struct {
		Env string `json:"env" maxLength:"262144" doc:".env file content (max 256 KB). Empty string deletes the file."`
	}
}
...
type ExecComposeInput struct {
	Name string `path:"name" maxLength:"128" doc:"Stack name"`
	Body struct {
		Command string `json:"command" minLength:"1" maxLength:"4096" doc:"Docker compose subcommand, quote-aware (e.g. ps, logs --tail 50 web, exec web sh -c 'env')"`
	}
}
...
type ImportStacksInput struct {
	Body struct {
		SourceDir string `json:"source_dir" minLength:"1" maxLength:"4096" doc:"Absolute path to directory containing stack subdirectories (e.g. /import/dockge)"`
	}
}
...
// UpdateStackCredentialsInput replaces the per-stack credentials atomically.
//
// This is full-replace semantics: send every field you want set, omit or use
// empty string for fields you want cleared. Unlike the pointer-based webhook
// update, there is no "keep current" option — POST the desired end state.
type UpdateStackCredentialsInput struct {
	Name string `path:"name" maxLength:"128" doc:"Stack name"`
	Body struct {
		Token      string `json:"token,omitempty" maxLength:"512" doc:"Git access token (empty string clears)"`
		SSHKey     string `json:"ssh_key,omitempty" maxLength:"16384" doc:"PEM SSH key (empty string clears)"`
		SSHKeyFile string `json:"ssh_key_file,omitempty" maxLength:"512" doc:"SSH key file path on the server (empty string clears)"`
		AgeKey     string `json:"age_key,omitempty" maxLength:"4096" doc:"Age private key for SOPS (empty string clears)"`
		Username   string `json:"username,omitempty" maxLength:"128" doc:"Basic auth username (empty string clears)"`
		Password   string `json:"password,omitempty" maxLength:"512" doc:"Basic auth password (empty string clears)"`
	}
}
